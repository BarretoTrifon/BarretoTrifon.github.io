...
document.addEventListener("click", (e) => {
  if (e.target === $btnSignIn || e.target === $btnSignUp) {
    ...
    if (e.target === $btnSignIn && $texto.value !== "") {
      ...
      $btnSignIni.innerHTML = "<i class='fa fa-refresh' aria-hidden='true'></i> Reiniciar sesion";
...
const sesion = (usuario) => {
  let usu = (document.getElementById("nombre").innerHTML =
    "Bienvenido  " + usuario + "   <i class='fas fa-user'></i>");
  // element.innerHTML = usu;
};

let x = document.getElementById("mensage");
let y = document.getElementById("conversor");
...
const cambioMo = () => {
  ...
  if (document.getElementById("dolar").checked) {
    ...
    document.getElementById("cantidad").innerHTML = "  " + resultado.toFixed(2)+"  Bs";
