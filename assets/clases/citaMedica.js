class Cita {
  ...
  leerDatoPaciente(citaRese) {
    ...
    let trif = citaRese.get("ci");
  
    let trifon = trif.slice(2);
  ...
  insertarCita(cita) {
    ...
    <td>Ficha:${cita.numeroFicha}   Hora:${cita.horaReseva}</td>
      ...
      <td>${cita.estadoPa}</td>
      <td>
          <a href="#" class="borrar-cita fa fa-trash" data-id="${cita.idCita}"></a>
      </td>
  ...
  leerLocalStorage() {
    ...
    citaLS.forEach(function (cita) {
      if (cita.pacienteci === txtci && cita.estadoPa === "ESPERA") {
        const row = document.createElement("tr");
        row.innerHTML = `
      <td>${cita.idCita}</td>
      <td>Ficha:${cita.numeroFicha}   Hora:${cita.horaReseva}</td>
